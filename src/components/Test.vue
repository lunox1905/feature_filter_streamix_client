  ...
  <div class="flex flex-column gap-1">
    <div class="p-fluid">
      <div class="p-fluid">
        <div class="p-inputgroup flex-1 mb-2">
          ...
          <InputText :value="selectedUrl" disabled />
          <Button icon="pi pi-copy" outlined @click="copy(selectedUrl)" v-tooltip.bottom="'Copy'" />
        </div>
      </div>
      ...
      <div class="flex flex-column gap-5">
        <div class="flex gap-5">
          <InputText id="domain" type="text" placeholder="s" v-model="inputFilter" />
          <Button :disabled="isLoading" @click="handleFilter" class="max-w-10rem flex justify-content-center">{{ 's' }}</Button>
        </div>
        <div class="flex">
          ...
          <div class="col-12 sm:col-6 md:col-3 flex">
            <div class="flex gap-2 align-items-center">
              ...
              <div class="white-space-nowrap">
                ...
                <span v-tooltip.top="'s'" class="pi pi-question-circle ml-1" />
              </div>
            </div>
          </div>
          ...
          >
            <div class="flex gap-2 align-items-center">
              ...
              <div v-else class="white-space-nowrap">
                ...
                <span v-tooltip.top="'d'" class="pi pi-question-circle ml-1" />
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="container-table">
      <div class="flex header-filter">
        <div class="left">#</div>
        <div>{{ totalComment + " " + 'd' }}</div>
      </div>
      <div class="flex flex-column comment-result">
        <div v-for="(cm, index) in comments" :key="cm.id" :class="`flex comment-main ${cm.winning && 'winning'}`">
          <div class="left">{{ page * rows - rows + 1 + index }}</div>
          <div class="flex flex-row gap-1 cm-row">
            <div class="avatar" :style="{ 'background-image': `url(${cm.author.avatar})` }">
              <img :src="cm.platform.icon" alt="" />
            </div>
            <div class="flex flex-column">
              <div class="font-semibold">{{ cm.author.name }}</div>
